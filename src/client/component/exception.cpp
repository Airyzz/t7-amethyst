...
namespace exception
{
	namespace
	{
		...
		void display_error_dialog()
		{
			...
			game::show_error(error_str.data(), "T7a ERROR");
		...
		void reset_state()
		{
			if (is_recoverable())
			{
				...
				game::Com_Error(game::ERR_DROP, "Fatal error (0x%08X) at 0x%p (0x%p).\nA minidump has been written.\n\n"
				                "T7a has tried to recover your game, but it might not run stable anymore.\n\n"
...
#pragma warning(pop)
		...
		{
			const std::string crash_name = utils::string::va("minidumps/t7a-crash-%s.zip",
			                                                 get_timestamp().data());

			utils::compression::zip::archive zip_file{};
			zip_file.add("crash.dmp", create_minidump(exceptioninfo));
			zip_file.add("info.txt", generate_crash_info(exceptioninfo));
			zip_file.write(crash_name, "T7a Crash Dump");
		}
